...
Для предотвращения роста таблицы **outbox** и утечек данных реализован **интеллектуальный механизм очистки**, основанный на водяных знаках (`watermarks`) и событиях присутствия (`presence`).

---

### 🔖 Основная идея

Каждый клиент во время работы обрабатывает сообщения, поступающие из **outbox**.  
Чтобы система понимала, **до какой позиции можно безопасно удалять данные**, используется **механизм водяных знаков**.

---

### 🧭 Как это работает

1. **Водяные знаки (`watermark`)**
    - При каждом успешном получении или подтверждении сообщения клиентом обновляется позиция (`lastProcessedOffset`).
    - Эти значения хранятся в реестре водяных знаков (`WatermarkRegistry`).

2. **Момент выхода клиента**
    - Когда пользователь покидает систему (disconnect, timeout, logout), компонент `LeaveStamp` фиксирует **отметку выхода** — timestamp последней активности клиента.
    - В этот момент создаётся *финальный водяной знак*, отражающий, до какого места клиент гарантированно получил сообщения.

3. **Сборщик мусора (`GarbageCollector`)**
    - Периодически запускается планировщик (`GarbageScheduler`), который анализирует все сохранённые водяные знаки.
    - Для каждого канала/пользователя вычисляется **минимальная позиция водяного знака** — это нижняя граница "живых" данных.
    - Все записи из `outbox`, чья позиция **меньше минимального watermark**, удаляются.

4. **Параллельная обработка**
    - Очистка выполняется **асинхронно и пакетно**, чтобы не мешать основным потокам сообщений.
    - Используется реактивная модель Mutiny (`Uni`/`Multi`), обеспечивающая управление скоростью и балансировку нагрузки.
